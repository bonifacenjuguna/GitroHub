...
The full history — every fix, what caused it, and what was verified-but-not-changed — now lives in **[CHANGELOG.md](CHANGELOG.md)**, kept separate so this README stays a clean overview instead of a scroll-forever wall of release notes.

**Current version: v0.8.2** — a debugging pass that fixed a crash-on-every-rename bug, a license-update display lag, and 3 more real issues found during a full follow-up audit (confirm-flow coverage, cache-invalidation completeness, dead code, orphaned callbacks). See [CHANGELOG.md](CHANGELOG.md#v082--debugging-pass-rename-crash-license-lag-and-3-more-real-bugs-found-by-a-full-audit) for the details.
