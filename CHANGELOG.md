...
**Jump to:** [v0.8.8](#v088--full-audit-pass-a-real-data-corruption-bug-and-19-other-fixes) · [v0.8.7](#v087--colors-redesigned-around-outcome-not-button-role) · [v0.8.6](#v086--button-color-remapping-after-seeing-it-live) · [v0.8.5](#v085--button-color-styling-bot-api-94) · [v0.8.4](#v084--memory-and-watchdog-hardening) · [v0.8.3](#v083--rename-crash-fix-callback-page-redesign-docs-cleanup) · [v0.8.2](#v082--deep-bug-sweep-on-the-v081-checkpoint) · [v0.8.1](#v081--stability-checkpoint-root-cause-pass-not-patches) · [v0.8.0](#v080--card-redesign-search-split-and-new-screens) · [v0.1.0–v0.7.2](#v010--v072--getting-the-bot-stable-click-to-expand)

---

---

### v0.8.8 — Full audit pass: a real data-corruption bug, and 19 other fixes

A dedicated audit of the entire codebase — every handler, scene, lib module, and doc file re-read start to finish, byte-level inspection where relevant — before trusting v0.8.7 as a base. Everything below is a root-cause fix, not a patch: each one addresses the underlying mechanism, applied everywhere that mechanism occurred, the same discipline as v0.8.1.

**Fixed (crashes):**
- `_executeSetLicense` (repoView.js) and `searchFiles` (browseFiles.js) both threw `ReferenceError: style is not defined` — `style` was only `require`'d inside a *different* function in the same file, not at module scope. License updates were succeeding on GitHub but showing a false failure; Search Files was fully broken on every match with no try/catch at all. Both now import `style` at module scope like every other handler; `searchFiles` also gained a try/catch it never had.

**Fixed (data corruption — the big one):**
- Every uploaded file — including binary files (images, PDFs, zips-within-zips, executables) — was getting silently corrupted on the way to GitHub. `scenes/uploadFile.js` decoded raw upload bytes with `buffer.toString('utf8')` before caching/hashing/committing them; any byte sequence that isn't valid UTF-8 (i.e. basically any binary file) gets lossily rewritten by that decode, and the corrupted version is what actually got pushed. Verified with a round-trip test: a 13-byte binary buffer came back 27 bytes and different in every byte. Fixed at the root — file content now stays a `Buffer` end-to-end (cache → local git-blob-sha comparison → GitHub commit), never touching a string encoding at all. Re-verified byte-identical after the fix.

**Fixed (data integrity):**
- Rename Repo never migrated `pinned_repos`, `repo_tags`, or `repo_path_memory` — all keyed by `repo_name` text — so renaming a pinned, tagged repo silently dropped it from ⭐ Pinned, orphaned its tags, and lost its upload-path memory, with zero notice. Delete Repo already had `cleanupOrphanedData()` for its equivalent problem; Rename now calls new `renameRepo()` functions added to `lib/pins.js`, `lib/tags.js`, and `lib/pathMemory.js` that move each row to the new name instead of leaving it stranded.
- Same class of orphaning could also happen invisibly whenever a repo was deleted *outside* GitroHub (straight from github.com) — Pinned already detected the mismatch (`if (!repo) continue`) but never actually cleaned up after itself. It now calls `cleanupOrphanedData()` for anything it finds orphaned while rendering the list.
- Upload/Replace had no optimistic-concurrency check, unlike Edit File (which re-checks its blob sha immediately before committing). Upload's file-vs-tree comparison happened once at "classify" time and then trusted that snapshot all the way to the actual commit — a real TOCTOU gap where someone else's change to the same path in between got silently clobbered. Upload now re-verifies every "modified" file's sha immediately before committing and stops with a clear message if anything drifted, matching Edit File's behavior.

**Fixed (double-tap protection was too coarse):**
- `actionLock` keys were action-only (`'deleteRepo'`, `'editFile'`, etc.) across all 13 call sites, not action+target — directly contradicting the module's own doc comment that unrelated actions shouldn't block each other. Deleting repo A then immediately repo B got B's action incorrectly blocked as "already processing." Every per-resource call site (Delete/Rename/Visibility/License/Edit File/Delete File/Fork/Upload Commit/Storage Clear) now keys on `${action}:${target}`; the dead 2-argument backward-compat branch (unreachable — every real call site already used 3 args) was removed at the same time.

**Fixed (Telegram limits):**
- Bulk Actions' progress message had no length guard — a large "Select All" could build a per-repo-line list past Telegram's 4096-character cap and throw uncaught on the very first `ctx.reply()`, with `bulkSelected` never cleared on that failure. Added a shared line-truncation helper and wrapped the whole flow in try/finally so the selection is always cleared, even if something above throws.
- File-tree buttons (`browse:dir:`, `file:edit:`, `file:delete:confirm:`, search results, etc.) embedded full repo file paths directly in `callback_data`, which Telegram caps at 64 bytes — a long or deeply-nested path could silently break the keyboard. Added `lib/pathTokens.js`, a bounded short-token registry keyed off the person's own session; every path-carrying button now carries an opaque token instead, resolved back to the real path server-side.

**Fixed (broken fallback links):**
- The "direct link instead" fallback shown when a repo zip is too big for Telegram (20MB cap) built an *unauthenticated* `github.com/.../archive/....zip` URL, which 404s for any private repo — exactly backwards, since private repos are also the ones most likely to be large enough to hit this path. Replaced with `github.getDownloadUrl()`, which requests GitHub's zipball endpoint with redirects disabled and hands back the short-lived, pre-signed Location header directly — works for private and public repos alike. Bulk Download's oversize path had no fallback link at all, unlike its two siblings (single-repo Download, External Repo Download) — it now gets one too, for all three flows consistently.

**Fixed (inconsistencies — design said one thing, code did another):**
- Both "🔄 Refresh" buttons (Activity Log, Settings) still sent a brand-new message on every tap instead of editing in place, the exact clutter pattern the rest of the bot was redesigned around avoiding (Bulk Select, Notifications, Cleanup, Pinned all already did this correctly). `showSettings()` gained the `edit` mode it was missing entirely; both refresh handlers now pass it.
- `config.MAX_TELEGRAM_FILE_SIZE_BYTES` was defined but never referenced — the 20MB Telegram-send-limit check was hand-duplicated as the magic number `20 * 1024 * 1024` in three separate files. All three now reference the one constant.
- `getTree`/`getTreeStats` support an optional `branch` param specifically to skip a redundant `getRepo()` lookup, but no caller passed it — including call sites that already had the repo object with `default_branch` sitting right there. `repoView.js`, `pinned.js`, and `myRepos.js` now pass it through; `repoCache.getTreeStats()` was updated to accept and forward it.
- `getLicenseText` and `downloadZip` were pure reads using `withAbortTimeout` (no retry) instead of `withRetry`, inconsistent with `github.js`'s own stated policy of retrying idempotent reads once on transient failure. Both switched.
- Direct SQL in the handler layer (`accessLogScreen.js`'s `toggleAlert`, `storageData.js`'s `setRetention`/`toggleAutoDelete`) broke the README's stated architecture rule that `lib/` is the data-access layer and `handlers/` is screen logic — every other handler already followed this. Moved into three new `lib/users.js` functions (`setActivityRetentionDays`, `toggleAutoCleanupOnDelete`, `toggleAlertOnNewConnection`).
- `activity.log()` calls were unguarded (no try/catch) at nearly every call site, including inside existing `catch` blocks — a transient Postgres blip *while logging a failure* could escape the surrounding catch entirely and become an unhandled rejection instead of the person just seeing the error message already in flight. Fixed once at the source: `activity.log()` itself now catches and logs its own failures instead of ever throwing, so every call site is safe without needing its own guard.
- `redisSessionStore.js`'s `get()` did `JSON.parse(raw)` with no try/catch — one malformed value in Redis threw on every single subsequent session read for that chat (session middleware runs on every interaction), effectively bricking the bot for that user. Now treated as "no session" instead — recoverable — rather than a permanent failure.
- `index.js`'s `main()` let a transient failure in `bot.telegram.setMyCommands()` (purely cosmetic — Telegram's slash-command popup) abort the entire boot sequence via `main().catch(() => process.exit(1))`. Wrapped in its own try/catch; boot now continues regardless.
- Timers in `withStepTimeout` (index.js) and `withTimeout` (redisSessionStore.js) were never `clearTimeout`'d once the real operation won the race — unlike `actionLock`/`fileBufferCache` elsewhere in the codebase, which are careful about this. Both now use `.finally(() => clearTimeout(timer))`; the session-store one runs on literally every Telegram update, so this was the hotter of the two leaks.
- Callback-page XSS footgun: `renderPage()` injected `JSON.stringify(data)` straight into an inline `<script>` tag without escaping `</script>` sequences. Nothing currently reaching it is user-influenced, but it's a classic footgun for the day something is. Now escapes any `</` in the serialized JSON.
- Three stacked 5-second caches for health/DB status: `app.js`'s own `healthCache` wrapped `pgDb.ping()`/`redisDb.ping()`, which are *already* independently 5s-cached — harmless, but not what the code read like it was doing. Removed the redundant outer layer.
- `createRepo.js`'s "⬅️ Back" moved the wizard's cursor correctly but only sent a bare "⬅️ Going back..." text with no actual question or keyboard attached, leaving the person looking at a stale screen with nothing to tap or type unless the earlier prompt happened to still be visible above. Each step's question is now a named, reusable renderer function; Back calls the same renderer the forward flow uses, so it actually redraws the prior prompt instead of just moving a pointer.

**Docs:**
- README's OAuth section claimed the requested scope was `repo` only, "nothing broader" — the code actually requests `repo, delete_repo` (correctly — Delete Repo and Bulk Delete need it). Corrected the doc to state what's actually requested and why.
- README's version badge said v0.7.2 while `package.json`/`.env.example`/`config.js` all said v0.8.7 — five releases stale. The separate "Known limitations (as of v0.8.2)" heading was independently stale too, three releases behind THAT. Badge corrected; the "Known limitations" heading no longer pins itself to a version number at all, since that's exactly what let it drift unnoticed for this long.
- Bumped to **v0.8.8** (`package.json`, `.env.example`, `config.js`).

**Investigated, not a codebase bug:** the reported logo/image issue — inspected `public/logo.png` at the byte level (PNG signature, chunk structure, IDAT/IEND boundaries); it's a structurally valid, uncorrupted 512×512 RGBA PNG. Serving code (`app.js`'s static middleware, `callback.html`'s `<img>` tag, its CSS) all check out, and the v0.8.3 fix for this exact bug class (`express.static` pointed at a file instead of a directory) is still in place. Most likely deployment-side (stale CDN/browser cache, file not actually deployed, wrong `BASE_URL`) rather than anything in this repo.
