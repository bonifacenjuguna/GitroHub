...
/**
 * 🆕 Scheduled Repos — a repo's full configuration (name, visibility,
 * description, license, gitignore) collected up front through the normal
 * Create Repo wizard, but with the actual GitHub creation deferred to a
 * future time. "Committed" here in the sense the person used it: the
 * point at which the repo (and its initial commit — GitHub's auto_init)
 * actually appears.
 */
async function create(telegramId, { name, description, visibility, license, gitignore, includeReadme, scheduledFor }) {
  const { rows } = await pool.query(
    `INSERT INTO scheduled_repos (telegram_id, name, description, visibility, license, gitignore, include_readme, scheduled_for)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
    [telegramId, name, description || null, visibility, license || null, gitignore || null, includeReadme !== false, scheduledFor]
  );
  return rows[0];
}
...
const EDITABLE_FIELDS = {
  ...
  license: 'license', gitignore: 'gitignore', scheduledFor: 'scheduled_for',
};
