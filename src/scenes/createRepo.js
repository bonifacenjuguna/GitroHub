...
const scene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    await ephemeral.sendEphemeral(ctx, '📦 New Repo — Step 2 of 6', bbtb.cancelWithBack);
  ...
  async (ctx) => {
    ...
    await ephemeral.sendEphemeral(ctx, '📦 New Repo — Step 4 of 6', bbtb.cancelWithBack);
    await ctx.reply(
  ...
  async (ctx) => {
    ...
    if (ctx.callbackQuery && ctx.callbackQuery.data.startsWith('create:readme:')) {
      ...
      await ctx.answerCbQuery();
      await ephemeral.sendEphemeral(ctx, '📦 New Repo — Step 5 of 6', bbtb.cancelWithBack);
      await ctx.reply(
  ...
  // Step 5 — receive license choice, ask .gitignore
  async (ctx) => {
    if (await handleGlobalActions(ctx)) return;
    if (ctx.callbackQuery && ctx.callbackQuery.data.startsWith('create:license:')) {
      ...
      await ctx.answerCbQuery();
      await ephemeral.sendEphemeral(ctx, '📦 New Repo — Step 6 of 6', bbtb.cancelWithBack);
      await ctx.reply(
        '🚫 Add a .gitignore? (or skip for none)',
        Markup.inlineKeyboard([
          [style.callback('Node', 'create:gitignore:Node'), style.callback('Python', 'create:gitignore:Python')],
          [style.callback('Java', 'create:gitignore:Java'), style.callback('Go', 'create:gitignore:Go')],
          [style.callback('Rust', 'create:gitignore:Rust'), style.callback('macOS', 'create:gitignore:macOS')],
          [style.callback('⌨️ Other (type it)', 'create:gitignore:custom')],
          [style.callback('⏭️ Skip', 'create:gitignore:none')],
        ])
      );
      return ctx.wizard.next();
    }
  ...
  // Step 6 — receive .gitignore choice (or await custom text), show confirm
  async (ctx) => {
    if (await handleGlobalActions(ctx)) return;

    // "⌨️ Other" was tapped — now waiting for a typed template name instead
    // of a button. Handled inside this same step, same pattern as the
    // scheduling custom-time input further down this file.
    if (ctx.wizard.state.awaitingGitignoreName) {
      if (ctx.message && ctx.message.text === '❌ Cancel') {
        delete ctx.wizard.state.awaitingGitignoreName;
        await ephemeral.sendEphemeral(ctx, 'Repo creation cancelled.', bbtb.mainMenu);
        return ctx.scene.leave();
      }
      if (!ctx.message || !ctx.message.text) {
        await ctx.reply('Send a .gitignore template name (e.g. "Swift", "Ruby"), or ❌ Cancel.');
        return;
      }
      delete ctx.wizard.state.awaitingGitignoreName;
      ctx.wizard.state.data.gitignoreTemplate = ctx.message.text.trim();
      return showCreateConfirm(ctx);
    }

    if (ctx.callbackQuery && ctx.callbackQuery.data.startsWith('create:gitignore:')) {
      const choice = ctx.callbackQuery.data.split('create:gitignore:')[1];
      await ctx.answerCbQuery();
      if (choice === 'custom') {
        await ephemeral.sendEphemeral(ctx, '⌨️ Type the .gitignore template name (GitHub\u2019s exact list is at github.com/github/gitignore).', bbtb.cancelOnly);
        ctx.wizard.state.awaitingGitignoreName = true;
        return;
      }
      ctx.wizard.state.data.gitignoreTemplate = choice === 'none' ? null : choice;
      return showCreateConfirm(ctx);
    }
    await ctx.reply('Tap a .gitignore option above.');
  },

  // Step 7 — confirm and create (or schedule for later)
  async (ctx) => {
    if (await handleGlobalActions(ctx)) return;
    ...
    const { name, isPrivate, description, includeReadme, licenseTemplate, gitignoreTemplate } = ctx.wizard.state.data;
    const actionLock = require('../lib/actionLock');
    const { skipped } = await actionLock.withLock(ctx.from.id, 'createRepo', async () => {
    try {
      const repo = await github.createRepo(token, { name, isPrivate, description, licenseTemplate, gitignoreTemplate });
...
/** Builds and shows the final confirm screen — shared by both the button-
 * picked and typed-custom-name .gitignore paths above, so the confirm
 * text is assembled in exactly one place either way. */
async function showCreateConfirm(ctx) {
  const { name, isPrivate, description, includeReadme, licenseTemplate, gitignoreTemplate } = ctx.wizard.state.data;
  const LICENSE_LABELS = { mit: 'MIT', 'apache-2.0': 'Apache 2.0', 'gpl-3.0': 'GPL v3', 'bsd-3-clause': 'BSD' };
  let text = `📦 ${name}\n${isPrivate ? '🔒 Private' : '🌐 Public'}`;
  if (description) text += `\n"${description}"`;
  text += `\n📄 README: ${includeReadme ? 'Yes' : 'Skip'}`;
  text += `\n⚖️ License: ${licenseTemplate ? (LICENSE_LABELS[licenseTemplate] || licenseTemplate) : 'None'}`;
  text += `\n🚫 .gitignore: ${gitignoreTemplate || 'None'}`;
...
async function finalizeSchedule(ctx, scheduledFor, tz) {
  ...
  await scheduledRepos.create(ctx.from.id, {
    ...
    gitignore: gitignoreTemplate || null,
    includeReadme,
    scheduledFor,
  });
  ...
  const user = await users.getUser(ctx.from.id);
  await ctx.reply(
    `📅 Scheduled: ${name} will be created ${timezone.formatInZone(scheduledFor, tz, { hour12: user.time_format === '12h' })} (${tz}).\n\n` +
    `Manage it anytime in 🤖 Automation → 🆕 Scheduled Repos.`,
    bbtb.mainMenu
  );
  return ctx.scene.leave();
}
