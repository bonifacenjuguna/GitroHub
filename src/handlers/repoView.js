  ...
  const { skipped } = await actionLock.withLock(ctx.from.id, 'deleteRepo', async () => {
    try {
      ...
      try {
        ...
        const retentionLabel = dbUser.trash_retention_days === 0 ? 'indefinitely' : `${dbUser.trash_retention_days}d`;
        ...
        const backupMsg = await ctx.replyWithDocument(
          ...
          { caption: `🗑️ Trash backup: ${repoName} (kept ${retentionLabel})` }
        );
        ...
        backupNote = ` — backed up to 🗑️ Trash for ${retentionLabel}`;
      } catch (backupErr) {
