...
async function showAutomationHub(ctx, { skipBbtb = false } = {}) {
  ...
  const [backupRulesList, scheduledPending, uploadsPending] = await Promise.all([
    backupRulesLib.listBackupRules(ctx.from.id),
    scheduledRepos.listPending(ctx.from.id),
    scheduledUploads.listPending(ctx.from.id),
  ]);
  ...
  const totalQueued = scheduledPending.length + uploadsPending.length;
  ...
  const text =
    ...
    `▸ 📅 *Schedule* — ${totalQueued} item${totalQueued === 1 ? '' : 's'} queued \\(🆕 Repos \\+ 📤 Uploads\\)\\. Also where 🌍 Timezone lives \\(currently *${format.escapeMd(user.timezone || 'UTC')}*\\)\\.\n` +
...
async function applySuggestedTag(ctx, repoName, tagId) {
  ...
  // suggestion card itself — not a new message, nothing to schedule
  if (!(await safeEditMessageText(ctx, `✅ Tagged ${repoName}${tag ? ` with ${tag.emoji} ${tag.name}` : ''}.`))) {
    await ephemeral.sendEphemeral(ctx, `✅ Tagged ${repoName}.`);
  }
}

async function dismissSuggestion(ctx) {
  // message too old to edit — non-fatal either way, it's just a suggestion
  await safeEditMessageText(ctx, '➖ Dismissed.');
}
